...
export function ApplicationsSkeleton({ viewMode = "table" }: ApplicationsSkeletonProps) {
  return (
    <div className="space-y-6 animate-pulse">
      ...
      {/* Filter Bar Skeleton */}
      <div className="p-4 rounded-2xl bg-white border border-slate-200/80 shadow-xs grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        <div className="h-10 bg-slate-100 rounded-xl lg:col-span-2" />
        <div className="h-10 bg-slate-100 rounded-xl" />
        <div className="h-10 bg-slate-100 rounded-xl" />
        <div className="h-10 bg-slate-100 rounded-xl" />
      </div>

      {/* Table vs Kanban Skeleton */}
      {viewMode === "table" ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[...Array(4)].map((_, colIdx) => (
            <div key={colIdx} className="bg-slate-50/80 rounded-2xl p-4 border border-slate-200/60 space-y-3">
              <div className="flex items-center justify-between pb-2 border-b border-slate-200/60">
                <div className="h-4 w-24 bg-slate-200 rounded" />
                <div className="h-5 w-6 bg-slate-200 rounded-full" />
              </div>
              {[...Array(2)].map((_, cardIdx) => (
                <div key={cardIdx} className="bg-white p-4 rounded-xl border border-slate-200 space-y-2 shadow-xs">
                  <div className="h-4 w-32 bg-slate-200 rounded" />
